...
class Pagination{
   ...
   function generateOutput(){
       ...
       $this->output = '<nav><ul>'.implode($this->options['delimiter'],$elements).'</ul></nav>';
   }
   function generate_link($page,$label=''){
       $url = str_replace('%page%',$page,$this->options['urlscheme']);
       if($label=='') $label=$page;
       $html = "<li><a ".(($this->options['focusedclass']!='' && $page == $this->options['page'])?"class=\"{$this->options['focusedclass']}\" ":"")."href=\"{$url}\">{$label}</a></li>";
       return $html;
   }
